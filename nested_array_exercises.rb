...
def count_empty_seats(chart)
  ...
  empty_seat_count = 0
   ...
   chart.each do |row|
     empty_seat_count += row.count(nil)
  end
...
def find_favorite(array_of_hash_objects)
  ...
  favorite_hash = array_of_hash_objects.find { |hash| 
  hash[:is_my_favorite?] == true}
  favorite_hash || {is_my_favorite?: true}
end

array_of_hash_object= [
